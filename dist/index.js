...
function favicons(source, options = {}, next = undefined) {
  ...
  function createFavicons(sourceset, platform) {
    ...
    const icons = Array.isArray(options.icons[platform]) ? options.icons[platform].reduce((opts, icon) => {
      // map the selected names to their original configs
      if (platform in config.icons && icon in config.icons[platform]) opts[icon] = config.icons[platform][icon];
      return opts;
    }, {}) : config.icons[platform];
    return Promise.all(Object.keys(icons || {}).map(name => createFavicon(sourceset, config.icons[platform][name], name, platformOptions)));
  }
...
function stream(params, handleHtml) {
  ...
  return through2.obj(function (file, encoding, callback) {
    ...
    favicons(file.contents, options).then(({
      images,
      files,
      html
    }) => {
      for (const asset of [...images, ...files]) {
        this.push(new File({
          path: asset.name,
          contents: Buffer.isBuffer(asset.contents) ? asset.contents : Buffer.from(asset.contents)
        }));
      }
